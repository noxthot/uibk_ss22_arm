...
b_lm <- lm(stunting ~ poly(mbmi, 3) + poly(agechild, 3), data=ZambiaNutrition)

## Estimate and account for heteroskedasticity
b_hetlm <- hetlm(stunting ~ poly(mbmi, 3) + poly(agechild, 3), data = ZambiaNutrition)


par(mfrow = c(2, 2), mar = c(4.1, 4.1, 4.1, 1.1))

termplot(b_hetlm$mean, term = 1, partial.resid = TRUE, se = TRUE, main = "Corrected", pch = ".")
termplot(b_lm, term = 1, partial.resid = TRUE, se = TRUE, main = "Uncorrected", pch = ".")
termplot(b_hetlm$mean, term = 2, partial.resid = TRUE, se = TRUE, main = "Corrected", pch = ".")
termplot(b_lm, term = 2, partial.resid = TRUE, se = TRUE, main = "Uncorrected", pch = ".")